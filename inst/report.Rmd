...
if (ce_table_exists) {
  ...
  list_of_SampleID <- unique(ce_results_display_dt$SampleID)
  
  message_to_user <- paste0(
    "The table below shows the detailed numerical results ",
    "of the commutability evaluation results of ",
    ifelse(
      length(list_of_SampleID) >= 3,
      ifelse(
        length(list_of_SampleID) >= 10,
        paste0(
          paste0(
            list_of_SampleID[1:3],
            collapse = ", "  
          ),
          "...",
          paste0(
            list_of_SampleID[(length(list_of_SampleID) - 2L):(length(list_of_SampleID) - 1L)],
            collapse = ", "
          ),
          " and ",
          list_of_SampleID[length(list_of_SampleID)],
          ". Because ten or more evaluated materials, the list is recycled."
        ),
        paste0(
          paste0(
            list_of_SampleID[1:(length(list_of_SampleID) - 1L)],
            collapse = ", "
          ),
          " and ",
          list_of_SampleID[length(list_of_SampleID)],
...
if (ce_plot_exists && ce_table_exists) {
  n_panels <- length(unique(params$ce_results$merged_ce_data$comparison))
