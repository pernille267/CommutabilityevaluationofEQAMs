...
(function() {
  console.error("!!! GLASS TABLE SCRIPT LOADED v13.0 !!!");

  // 1. GLOBAL INITIALIZATION (Runs on new content)
  // We use a MutationObserver or just bind to Shiny events to init pagination
  // because pagination controls need to be built immediately, not just on click.
  ...
  $(document).on('shiny:connected shiny:sessioninitialized shiny:value', function() {
    setTimeout(attemptInit, 100); // Slight delay to ensure DOM is ready
  });
