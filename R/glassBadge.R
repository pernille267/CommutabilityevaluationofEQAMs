...
#'              Options: "green", "orange", "blue", "red", "purple", "teal", "gray", "gold".
#'              Alternatively, can be a custom hex code.
#' @param shape The shape of the badge. Options: "pill", "circle", "square", "star", "hexagon".
...
glassBadge <- function(label,
  ...
  tooltip_class <- if (!is.null(tooltip)) paste0("gb-tooltip-trigger-", tooltip_mode) else ""

  final_class <- paste(base_class, color_class, shape_class, size_class, glow_class, tooltip_class)

  # --- Inline Style for Custom Hex Colors ---
  # If color isn't one of the presets, we apply it as a CSS variable
  known_colors <- c("green", "orange", "blue", "red", "purple", "yellow", "gray", "gold")
  style_attr <- NULL
  if (!color %in% known_colors) {
    # Assume it's a hex code or valid CSS color
    style_attr <- sprintf("--gb-custom-color: %s;", color)
    # We still add a generic class to handle the variable usage
    final_class <- paste(final_class, "gb-color-custom")
  }

  # --- Star/Hexagon SVG Wrappers ---
  # If shape is complex, we might need a specific container or SVG clip-path.
  # For simplicity, we stick to CSS-based shapes, but 'star' needs a unicode or icon content usually.
  ...
  if (!is.null(tooltip)) {
    tag_badge <- htmltools::tagAppendAttributes(tag_badge, `data-tooltip` = tooltip)
  }
