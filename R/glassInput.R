#' Glass Text Input
#'
#' A fancy, glass-morphism styled text input.
#'
...
#' @export
glassTextInput <- function(inputId,
                           label,
                           value = "",
                           label_icon = NULL,
                           help_text = NULL,
...
#' Glass Numeric Input
#'
#' A fancy, glass-morphism styled numeric input with range validation.
#'
...
#' @param accept A numeric vector of length 2 \code{c(min, max)} defining the acceptable range.
#' @param warning_unacceptable Text to display if value is outside \code{accept} range.
#' @param disabled Logical.
#'
#' @export
glassNumericInput <- function(inputId,
                              label,
                              value,
                              min = NA,
                              max = NA,
                              ...
                              unit = NULL,
                              accept = c(-Inf, Inf),
                              warning_unacceptable = "Value is outside acceptable range",
                              disabled = FALSE) {
...
glassInput <- function(inputId,
  ...
  unit_html <- if (!is.null(unit)) htmltools::tags$span(class = "glass-input-unit", unit) else NULL
