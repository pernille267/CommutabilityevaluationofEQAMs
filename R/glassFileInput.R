#' Glass File Input
#'
#' A fancy, glass-morphism styled file input with animated progress and tooltips.
#' Built completely from scratch to avoid AdminLTE conflicts.
#'
#' @param inputId The input slot that will be used to access the value.
#' @param label Display label for the control, or NULL for no label.
#' @param label_icon Optional icon() to place next to the label.
#' @param accept A character vector of MIME types or file extensions (e.g. c(".csv", ".xlsx")).
#' @param button_label The text on the browse button.
#' @param button_label_icon An icon() for the browse button.
#' @param placeholder The text to display before a file is selected.
#' @param tooltip_empty Text to show when hovering the text area before upload.
#' @param tooltip_filled Text to show when hovering the text area after file selection.
#' @param help_text Text to show when hovering the help-icon.
#' @param width The width of the input.
#'
#' @importFrom htmltools tagList tags htmlDependency
#' @export
glassFileInput <- function(inputId,
                           label,
                           label_icon = NULL,
                           accept = NULL,
                           button_label = "Browse...",
  ...
  accept_str <- if (!is.null(accept)) gsub(" ", "", paste(accept, collapse = ",")) else ""
  assets_path <- system.file("assets", package = "CommutabilityevaluationofEQAMs")
  ...
  ui <- htmltools::tags$div(
    ...
    # 1. THE HEADER (Mimicking .glass-card-header)
    htmltools::tags$div(
    ...
    htmltools::tags$div(
      ...
      htmltools::tags$div(
        ...
        if (!is.null(button_label_icon)) htmltools::tags$span(class = "btn-icon", button_label_icon),
      ...
      htmltools::tags$div(
        ...
        `data-state` = "empty",
        htmltools::tags$span(class = "glass-file-filename", `data-placeholder` = placeholder, placeholder)
      ),
...
#'
#' Since we aren't using Shiny's native uploader, the input returns a list
#' containing the Base64 content. This helper saves it to a temp file
#' so it behaves exactly like a standard fileInput.
#'
...
processGlassFile <- function(input_data) {
  ...
  # Decode Base64
  # Requires 'jsonlite' or 'base64enc'. Using internal raw conversion if possible,
  # but assuming jsonlite is available as it's a shiny dependency.
  ...
  # Create temp file
  tmp_path <- tempfile(fileext = paste0(".", tools::file_ext(input_data$name)))

  # Decode and write
  # You might need: install.packages("base64enc")
