...
#' @param type The type of notification: "message", "success", "warning", or "error".
#' @param duration Duration in milliseconds before auto-dismiss. Set to NULL or 0
...
#' @param id Optional unique ID for the toast. If not provided, one is generated.
#'   Use this to later remove a specific toast with `removeGlassToast()`.
#' @param icon Optional custom icon HTML. If not provided, a default icon is used
#'   based on the type.
...
showGlassToast <- function(message,
                           title = NULL,
                           type = c("message", "success", "warning", "error"),
                           duration = 4000,
                           closeButton = TRUE,
                           id = NULL,
                           icon = NULL,
  ...
  if (is.null(id)) {
    id <- paste0("toast-", as.integer(round(as.numeric(Sys.time()))) * 1000, "-", sample(1000:9999, 1))
  }
  ...
  if (!is.null(title)) {
    msg$title <- as.character(title)
  }

  if (!is.null(duration) && duration > 0) {
    msg$duration <- as.numeric(duration)
  }
  ...
  session$sendCustomMessage("glassToast", msg)
...
removeGlassToast <- function(id, session = shiny::getDefaultReactiveDomain()) {
  session$sendCustomMessage("glassToast", list(
    action = "remove",
    id = id
  ))
}
